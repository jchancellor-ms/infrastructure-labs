...
$targetFileSourceExport = "c:\temp\logtesting\logtest11\source_export.csv"
$targetFileLawInventory = "c:\temp\logtesting\logtest11\law_inventory.csv"
$lookbackDays = 30
$largeTableSize = 50
    ...
    foreach($table in $tables.results) {
        ...
        if ([float]$table.tableAverage -gt 0) {
            ...
            While($lookBackDaysThis -gt 0) {
                ...
                #$sum = 0
                if ($results.results) {
                        $results.results | export-csv $targetFileSourceExport -append
                }   
